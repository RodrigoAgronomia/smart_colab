...
class EESPNet_Feat(nn.Module):
    ...
    def forward(self, input, p=0.2, seg = False):
        '''
        :param input: Receives the input RGB image
        :return: a C-dimensional vector, C=# of classes
        '''
        ...
        return out_l1, out_l2, out_l3, out_l4, out_l5


class EESPNet_Seg(nn.Module):
    def __init__(self, classes=20, s=1, gpus=1, channels=3):
        super().__init__()
        ...
        self.net = EESPNet_Feat(classes, s=s, channels=channels)
        self.proj_L5_C = CBR(self.net.level5[-1].act.num_parameters, self.net.level4[-1].module_act.num_parameters, 1, 1)
        self.proj_L4_C = CBR(2*self.net.level4[-1].module_act.num_parameters, self.net.level3[-1].module_act.num_parameters, 1, 1)
    ...
    def forward(self, input, ret_feat = False):
        out_l1, out_l2, out_l3, out_l4, out_l5 = self.net(input, seg=True)
        
        
        out_l5_proj = self.proj_L5_C(out_l5)
        up_l5_to_l4 = F.interpolate(out_l5_proj, scale_factor=2, mode='bilinear', align_corners=True)
        merged_l4_upl5 = torch.cat([out_l4, up_l5_to_l4], 1)
        out_l4_proj = self.proj_L4_C(merged_l4_upl5)
